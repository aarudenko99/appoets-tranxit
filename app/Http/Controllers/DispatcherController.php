...
class DispatcherController extends Controller
{
    ...
    public function mark_as_read(Request $request) {
        ...
        Session::set('lastReadTime', \Carbon\Carbon::now());
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function new_trips(Request $request)
    {
        $Trips = UserRequests::with('user', 'provider')
                    ->where('status', "SEARCHING")
                    ->limit(1);

        if (Session::get('lastReadTime') != null)
            $Trips = $Trips->where('created_at', '>', Session::get('lastReadTime'));
