...
class SendPushNotification extends Controller
{
     ...
     * @return void
     */
    public function provider_reminder($provider, $schedule_at){

        return $this->sendPushToProvider($provider, trans('api.push.schedule_reminder', ['date'=>$schedule_at]));
    	...
    	try{
            ...
            if($user->device_token != ""){
    	    	...
    	    	if($user->device_type == 'ios'){
                    ...
                    return$this->sendPushToCurl($user->device_token,$push_message);
    	...
    	try{
            ...
            if($provider->token != ""){
            	...
            	if($provider->type == 'ios'){
                    ...
                    return $this->sendPushToCurl($provider->token,$push_message);
    ...
    public function sendPushToCurl($token, $push_message){
        ...
        $headers = array( 
            'Authorization: key='.env('ANDROID_PROVIDER_PUSH_KEY'), 
        ...
        $result = curl_exec($ch);
        \Log::info('sending push for provider : '. $result);
