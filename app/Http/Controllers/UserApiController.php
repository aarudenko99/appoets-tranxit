...
/* 	public function mail()
{
    ...
    Mail::send('emails.mailExample', $user, function($message) use ($user) {
        $message->to('kumarvj669@gmail.com');
	...
	public function business_hour(Request $request){

		$start_hour = Setting::get('start_hour');
		$end_hour = Setting::get('end_hour');
		$returnarr = array('start_hour'=>$start_hour,'end_hour'=>$end_hour);
		if(strtotime($request->current_time) < strtotime($start_hour) || strtotime($request->current_time) > strtotime($end_hour))
		{
    ...
    public function is_business_hour(){

        $start_hour = Setting::get('start_hour');
        $end_hour = Setting::get('end_hour');

        $current_time = Carbon::now();
        if(strtotime($current_time) < strtotime($start_hour) || strtotime($current_time) > strtotime($end_hour))
        {
            return false;
        }
        else{
    ...
    public function send_request(Request $request) {
        ...
        $this->validate($request, [
                ...
                'payment_mode' => 'required|in:CASH,CARD,WALLET,PAYPAL',
            ]);

        if ($request->payment_mode == 'WALLET') {
            // $request->payment_mode = 'CASH';
            $request->use_wallet = 1;
        }
        ...
        try{
            ...
            $otp3 = substr($request->d_address, 0, 1);
            $otp = $otp1.$otp2.$otp3;


            $UserRequest->user_id = Auth::user()->id;
            $UserRequest->estimated_fare = Auth::user()->estimated_fare_tmp;
    ...
    public function rate_provider(Request $request) {
        ...
        if ($UserRequests) {
            if($request->ajax()){
                return response()->json(['message' => trans('api.user.not_paid')], 500);
            } else {
                return back()->with('flash_error', 'Service Already Started!');
            }
        }
    ...
    public function estimated_fare(Request $request){
        ...
        if (!$this->is_business_hour()) {

            $start_hour = Setting::get('start_hour');
            $end_hour = Setting::get('end_hour');
            
            return response()->json(['error' => trans('api.business_time', ['from' => $start_hour, 'to' => $end_hour])], 500);
        }
        ...
        try{
            ...
            */ 
            ...
            $user = Auth::user();
            $user->estimated_fare_tmp = round($total,2);
            $user->save();
